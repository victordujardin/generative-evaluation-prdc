...
# Create a single figure for density metrics
...
plt.savefig("density_comparison_mean_variance.png", dpi=300, bbox_inches='tight')
...
# Similarly, you can plot Coverage and Weighted Coverage if needed
# Example for Coverage Naeem
plt.figure(figsize=(10, 6))
...
plt.fill_between(k_values,
                 np.array(coverage_means) - np.sqrt(coverage_vars),
                 np.array(coverage_means) + np.sqrt(coverage_vars),
...
plt.legend()
plt.grid(True)
plt.savefig("coverage_naeem_mean_variance.png")
# plt.show()

# Example for Weighted Coverage
plt.figure(figsize=(10, 6))
...
plt.fill_between(k_values,
                 np.array(weighted_coverage_means) - np.sqrt(weighted_coverage_vars),
                 np.array(weighted_coverage_means) + np.sqrt(weighted_coverage_vars),
...
plt.legend()
plt.grid(True)
plt.savefig("weighted_coverage_mean_variance.png")
# plt.show()