...
def generate_real(m, lowrank, beta,epsilon, addvar = True):
    # np.random.seed(seed)
...
def generate_fake(m, lowrank,beta ,epsilon, addvar = True):
...
def generate(m, lowrank, beta, epsilon, addvar = True):
    # np.random.seed(seed)
    X = np.random.multivariate_normal(np.zeros(lowrank), np.identity(lowrank), size=m)
    X = pd.DataFrame(X, columns=[f'X_{j}' for j in range(lowrank)])
